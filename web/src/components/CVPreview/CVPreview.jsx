...
const CVPreview = React.forwardRef(({ cvData, onSave, personalInfo }, ref) => {
    ...
    const parseCvData = (data) => {
        ...
        // Create a proper experience array from transferableExperience or experience
        let experienceArray = [];
if (Array.isArray(parsedData.experience)) {
    ...
    if (Array.isArray(parsedData.transferableExperience)) {
        ...
        // Handle single experience case if needed
        experienceArray = [{
        ...
        // Handle skills format (could be array or object with technical/soft)
        let skillsData = [];
        ...
        return {
            fullName: personalInfo.fullName || personalInfo.name || personalInfo.personalInfo?.fullName || '',
            contact: {
                email: personalInfo.contact?.email || personalInfo.email || personalInfo.personalInfo?.email || '',
                phone: personalInfo.contact?.phone || personalInfo.phone || personalInfo.personalInfo?.phone || '',
                linkedin: personalInfo.contact?.linkedin || personalInfo.linkedin || personalInfo.personalInfo?.linkedin || '',
                github: personalInfo.contact?.github || personalInfo.github || personalInfo.personalInfo?.github || '',
                portfolio: personalInfo.contact?.portfolio || personalInfo.portfolio || personalInfo.personalInfo?.portfolio || '',
            },
            professional_summary: parsedData.professionalSummary || parsedData.professional_summary || '',
...
const addEducation = () => {
    ...
    const normalizedData = {
        ...
        professional_summary: displayData.professional_summary || displayData.professionalSummary || '',
    ...
    const isSoftSkill = (skill) => {
        ...
        const softSkills = [
            ...
            'interpersonal', 'negotiation', 'critical thinking', 'emotional intelligence',
            'team player', 'active listening', 'conflict resolution', 'presentation',
            'mentoring', 'coaching', 'decision making', 'strategic thinking'
        ];
        return softSkills.some(softSkill => skillStr.includes(softSkill.toLowerCase()));
    };
    ...
    if (isEditing) {
        return (
            <div ref={ref} className={styles['cv-container']}>
                ...
                {/* PROFESSIONAL SUMMARY SECTION - EDIT MODE */}
                <div className={styles['form-group']}>
...
<div>
        ...
        <div key={expIndex} className={styles['section-container']}>
            ...
            <div className={styles['form-group']}>
                ...
                {exp.bulletPoints?.map((point, bulletIndex) => (
                    <div key={bulletIndex} className={styles['bullet-point-container']}>
                        <textarea
                            value={point}
                        ...
                        <div key={index} className={styles['section-container']}>
                            ...
                            <div className={styles['small-form-group']}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleArrayChange('projects', index, 'deployedLink', e.target.value)}
                                    className={styles.input}
                                />
                            </div>
                            <div className={styles['small-form-group']}>
                            ...
                            <div className={styles['small-form-group']}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleArrayChange('projects', index, 'technologiesUsed', e.target.value.split(',').map(tech => tech.trim()))}
                                    className={styles.input}
                                />
                            </div>
                            <button 
                        ...
                        <div key={index} className={styles['section-container']}>
                            <div className={styles['small-form-group']}>
                                ...
                                <input
                                    type="text"
                                    value={edu.program || ''}
                                    onChange={(e) => handleArrayChange('education', index, 'program', e.target.value)}
                                    className={styles.input}
                                />
                            </div>
                            <div className={styles['small-form-group']}>
                            ...
                            <div className={styles['small-form-group']}>
                                ...
                                <textarea
                                    ...
                                    onChange={(e) => handleArrayChange('education', index, 'highlights', e.target.value)}
                                    className={styles['small-textarea']}
                                />
                            </div>
                            <button 
    ...
    return (
        <div ref={ref} className={styles['cv-container']}>
            ...
            {/* NAME AND PERSONAL INFO SECTION */}
            <div className={styles['name-section']}>
                <h1 className={styles['cv-title']}>{fullName || 'Professional Profile'}</h1>
                <div className={styles['personal-details']}>
                    ...
                    {contact.linkedin && (
                        <p className={styles['contact-info']}>
                            ...
                            >
                                {contact.linkedin.replace(/^https?:\/\/(www\.)?linkedin\.com\//i, 'linkedin.com/')}
                            </a>
                        </p>
                    )}
                    {contact.github && (
        ...
        <div className={styles['skills-container']}>
            <div className={styles['skills-column']}>
                ...
                <div className={styles['skills-list']}>
                    {skills.filter(skill => !isSoftSkill(skill)).map((skill, index) => (
                        <span key={`technical-${index}`} className={styles['skill-item']}>
                            {skill}
                        </span>
                    ))}
                </div>
        ...
        <div className={styles['projects-list']}>
            {projects.map((project, index) => (
                <div key={index} className={styles['project-item']}>
                    <h3 className={styles['project-name']}>{project.name || 'Project Name Not Specified'}</h3>
                    {project.description && (
                    ...
                    <div className={styles['project-links-container']}>
                        {project.deployedLink && (
                            <a
                                href={project.deployedLink.startsWith('http') ? project.deployedLink : `https://${project.deployedLink}`}
                                target="_blank"
                                rel="noopener noreferrer"
                    ...
                    {project.technologiesUsed && project.technologiesUsed.length > 0 && (
                        <div className={styles['technologies-container']}>
                            <span className={styles['technologies-label']}>Technologies: </span>
                            <div className={styles['technologies-list']}>
                                {project.technologiesUsed.map((tech, techIndex) => (
                                    <span key={techIndex} className={styles['technology-tag']}>
                                        {tech}
                                    </span>
                                ))}
                            </div>
